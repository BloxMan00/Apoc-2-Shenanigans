...
local is_synX, line_mt = false, nil
pcall(function()
    if islclosure(newDrawing) == true then
        is_synX = true
        line_mt = getupvalue(newDrawing, 4)
    end
end)

local newrender = is_synX and getupvalue(newDrawing, 1) or newDrawing

local destroy = (is_synX and line_mt) and getupvalue(line_mt.__index, 3) or function(drawing)
    drawing:Remove()
end

local setproperty = (is_synX and line_mt) and getupvalue(line_mt.__newindex, 4) or setrenderproperty or function(object, property, value)
    object[property] = value
end

local getproperty = (is_synX and line_mt) and getupvalue(line_mt.__index, 4) or getrenderproperty or function(object, property)
